...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    //只显示年
    self.messageLabel.text = [formatter stringFromDate:self.mSelectDate];
    // 2.设置选择器中间选中行的样式
...
-(UILabel *)messageLabel{
    if (!_messageLabel) {
        ...
        _messageLabel.textColor = [UIColor colorWithRed:233/255.0
                                                  green:237/255.0
                                                   blue:242/255.0
                                                  alpha:0.9];
        ...
        [_messageLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.edges.equalTo(self.pickerView);
        }];
        [self.alertView sendSubviewToBack:_messageLabel];
