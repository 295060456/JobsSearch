    ...
    switch (self.orientation) {
        ...
        case BV_Orientation_Horizontal: {
            ...
            if (_orginPageCount > 1) {
                ...
                if (self.isCarousel) {
                    // 滚到第二组第一张
                    [self.insideScrollView setContentOffset:CGPointMake(_pageSize.width * _orginPageCount, 0)
                                                   animated:NO];
                    ...
                    // 滚到开始
                    [self.insideScrollView setContentOffset:CGPointMake(0, 0)
                                                   animated:NO];
                    _timerPageIndex = _orginPageCount;
                }
            }break;
...
- (void)scrollToPageWithPageNumber:(NSUInteger)pageNumber {
    if (pageNumber < _pageCount) {
        ...
        switch (self.orientation) {
            // 横向
            case BV_Orientation_Horizontal: {
                [self.insideScrollView setContentOffset:CGPointMake(_pageSize.width * _timerPageIndex,
                                                                    0)
                                               animated:YES];
                break;
            }
            ...
            case BV_Orientation_Vertical: {
                [self.insideScrollView setContentOffset:CGPointMake(0,
                                                                    _pageSize.height * _timerPageIndex)
                                               animated:YES];
...
- (void)bsvStartTimer {
    if (_orginPageCount > 1 &&
        ...
        self.isCarousel) {
        _bannerTimer = [NSTimer scheduledTimerWithTimeInterval:self.autoTime
                                                        target:self
...
//    NSLog(@"_timerPageIndex ==> %ld", _timerPageIndex);
    switch (self.orientation) {
            // 横向
        case BV_Orientation_Horizontal: {
            [self.insideScrollView setContentOffset:CGPointMake(_timerPageIndex * _pageSize.width,
                                                                0)
                                           animated:YES];
            break;
        }
        ...
        case BV_Orientation_Vertical: {
            [self.insideScrollView setContentOffset:CGPointMake(0,
                                                                _timerPageIndex * _pageSize.height)
                                           animated:YES];
            break;
        }default:
            break;
    ...
    switch (self.orientation) {
        ...
        case BV_Orientation_Horizontal: {
            ...
            endIndex = MIN(endIndex + 1,
                           self.cardViewArray.count - 1);
            // 定制visibleRange
            _visibleRange = NSMakeRange(startIndex,
...
- (void)bsvAddCardViewToInsideScrollViewWithIndex:(NSInteger)pageIndex {
    ...
    if ((NSObject *)cardView == [NSNull null]) {
        cardView = [_dataSource bannerView:self
              cardViewForBannerViewAtIndex:pageIndex % _orginPageCount];
        NSAssert(cardView != nil, @"cardView不能为nil");
        [self.cardViewArray replaceObjectAtIndex:pageIndex
                                      withObject:cardView];
        
        // 添加点击手势
        UITapGestureRecognizer *singleTap = [[UITapGestureRecognizer alloc] initWithTarget:self
                                                                                    action:@selector(bsvTapCardView:)];
        ...
        switch (self.orientation) {
                // 横向
            case BV_Orientation_Horizontal: {
                cardView.frame = CGRectMake(_pageSize.width * pageIndex,
                                            0,
                                            _pageSize.width,
                                            _pageSize.height);
...
- (void)bsvTapCardView:(UIGestureRecognizer *)ges {
    if ([self.delegate respondsToSelector:@selector(bannerView:didSelectCardView:withCardViewIndex:)]) {
        [self.delegate bannerView:self
                didSelectCardView:ges.view
                withCardViewIndex:ges.view.tag];
    }
}
...
- (void)bsvRemoveCardViewFromCardViewArrayWithIndex:(NSInteger)index {
    ...
    [self.cardViewArray replaceObjectAtIndex:index
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if ([_delegate respondsToSelector:@selector(bannerView:didScrollToCardViewWithIndex:)] &&
        ...
        [_delegate bannerView:self
 didScrollToCardViewWithIndex:pageIndex];
    }
